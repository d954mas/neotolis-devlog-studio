...
def process_beat_audio(
    ...
    r = _run(
        f'ffmpeg -y -i "{rec_path}" -vn -ac 1 -ar 48000 '
        f'-af "silenceremove=start_periods=1:start_duration=0.2:start_threshold=-40dB,'
        f'highpass=f=60:poles=2,adeclip" '
        f'-c:a pcm_s16le "{out_pre}" 2>&1'
    )
    ...
    r = _run(
        f'ffmpeg -i "{out_pre}" -af '
        f'"loudnorm=I={loudness_target}:TP={true_peak}:LRA={lra}:print_format=json" '
        f'-f null - 2>&1'
    )
