...
def test_no_scenes_returns_empty():
    ...
    segs, warns = build_segments(chunks, wins, None, 10.0, D, {})
    assert segs == []
    assert warns == []
...
def test_single_default_scene_spans_audio():
    ...
    chunks = [_pl(), _pl(), _pl()]
    wins = [(0.0, 3.0), (3.0, 6.0), (6.0, 10.0)]
    segs, _ = build_segments(chunks, wins, default, 10.0, D, {})
    assert len(segs) == 1
...
def test_first_chunk_with_scene_after_t0_creates_gap():
    ...
    segs, _ = build_segments(chunks, wins, None, 15.0, D, {})
    assert len(segs) == 1
...
def test_same_src_consecutive_chunks_merge_first_offset_wins():
    ...
    segs, _ = build_segments(chunks, wins, None, 10.0, D, {})
    assert len(segs) == 1
...
def test_plate_chunks_between_scenes_extend_current():
    ...
    segs, _ = build_segments(chunks, wins, None, 8.0, D, {})
    assert len(segs) == 1
...
def test_audio_dur_caps_last_segment():
    ...
    chunks = [_pl(s)]
    wins = [(0.0, 27.42)]
    segs, _ = build_segments(chunks, wins, None, 27.42, D, {})
    assert len(segs) == 1
...
def test_imageshot_content_is_its_own_segment():
    ...
    segs, _ = build_segments(chunks, wins, Scene(kind="video", src="bg.mp4"),
                             6.0, D, {})
...
def test_videoshot_content_is_video_segment():
    ...
    wins = [(0.0, 5.0)]
    segs, _ = build_segments(chunks, wins, None, 5.0, D, {})
    assert len(segs) == 1
...
def test_segment_boundaries_get_default_crossfade():
    a = Scene(kind="image", src="a.png")
    b = Scene(kind="image", src="b.png")
    segs, _ = build_segments([_pl(a), _pl(b)], [(0.0, 4.0), (4.0, 10.0)],
                             None, 10.0, D, {})
...
def test_chunk_transition_overrides_segment_xfade():
    ...
    segs, _ = build_segments(chunks, [(0.0, 4.0), (4.0, 10.0)], None, 10.0, D, {})
...
def test_video_offset_past_eof_clamped_with_warning():
    ...
    segs, warns = build_segments([_pl(scene)], [(0.0, 3.0)], None, 3.0, D, assets)
...
def test_video_offset_clamped_to_zero_floor_when_window_longer_than_source():
    ...
    segs, warns = build_segments([_pl(scene)], [(0.0, 10.0)], None, 10.0, D, assets)
...
def test_video_offset_within_source_no_warning():
    ...
    assets = {"ok.mp4": probe("ok.mp4", "video", duration=30.0,
                              width=1080, height=1920)}
    segs, warns = build_segments([_pl(scene)], [(0.0, 5.0)], None, 5.0, D, assets)
