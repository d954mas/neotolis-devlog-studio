...
def check_performance_contract(hooks: object) -> GateResult:
    details: list[str] = []
...
def check_generated_client(
    ...
    *,
    ...
    try:
        relative_webui = webui_root.relative_to(repo_root).as_posix()
    except ValueError:
        relative_webui = webui_root.as_posix()
    before_rc, before = _git_snapshot(repo_root, relative_webui, env=env)
    npm = "npm.cmd" if os.name == "nt" else "npm"
    generate_rc = run([npm, "run", "generate:client"], webui_root, env)
    after_rc, after = _git_snapshot(repo_root, relative_webui, env=env)
    details: list[str] = []
    if before_rc or after_rc:
        details.append("git status failed while checking generated client cleanliness")
    if generate_rc:
        details.append(f"generate:client exited {generate_rc}")
    if before != after:
        details.append("generate:client changed the working tree; commit regenerated output")
    return GateResult(
        name="generated-openapi-client",
        status=GateStatus.FAIL if details else GateStatus.PASS,
        details=tuple(details),
